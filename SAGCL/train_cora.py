...
def run_SAGCL(args, gpu_id=None, **kwargs):
    # ===================================================#
    ...
    for current_iter, epoch in enumerate(tqdm(range(args.start_epoch, args.start_epoch + args.epochs + 1))):
        ...
        s_p = F.pairwise_distance(h_a, h_p)  #gnn_2 10
        s_p_1 = F.pairwise_distance(h_a, h_p_1)  #llm_1 10
        s_p_2=F.pairwise_distance(h_p, h_p_1)   #llm_2
        s_n_list = []
        s_n_list_1 = []
        for h_n in idx_list:
        ...
        if epoch % args.epochs == 0 and epoch != 0:
            model.eval()
            h_a, h_p = model.embed(h_p_1,feature_X, A_I_nomal)
            #embs = h_p
            embs = h_p
            ...
            string_2 = Fore.GREEN + " epoch: {},ACC: {:.1f}±{:.2f}, NMI: {:.1f}±{:.2f}, ARI: {:.1f}±{:.2f} ".format(
                epoch,
                accs.mean().item(), accs.std().item(),
                nmis.mean().item(), nmis.std().item(),
                aris.mean().item(), aris.std().item()
            )
            result_str = " epoch: {},ACC: {:.1f}±{:.2f}, NMI: {:.1f}±{:.2f}, ARI: {:.1f}±{:.2f}\n".format(
                epoch,
                accs.mean().item(), accs.std().item(),
                nmis.mean().item(), nmis.std().item(),
                aris.mean().item(), aris.std().item()
            ...
            with open('cora_7.31.txt', 'a') as f:
                f.write(result_str+'\n')
                f.write(str("margin1:") + str(args.margin1)+'\n')
                f.write(str("margin2:")+str(args.margin2)+'\n')
